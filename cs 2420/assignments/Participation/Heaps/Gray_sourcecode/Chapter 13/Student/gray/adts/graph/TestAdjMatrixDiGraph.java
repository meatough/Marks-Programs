...
public class TestAdjMatrixDiGraph extends TestCase {
  ...
  public void testConstructor() {
    AdjMatrixDiGraph<String> g = new AdjMatrixDiGraph<String> ();
    assertEquals( 0, g.getNumberOfVertices() );
    assertEquals( 0, g.getNumberOfEdges() );
  }

  /**
   * Verify that we can add vertices to a graph and get the
  ...
  /**
   * Verify that we can add vertices and to a graph and get the
   * correct Vertex and Edge count.
   */
  ...
  public void testAddDuplicateEdge() {
    ...
    try {
      graph.addEdge( v2, v1 );
      this.fail( "duplicate edge exception not caught" );
    }
  ...
  /**
   * Verify that we can get the neighbors of a vertex. That is, the
   * adjacent vertices.
   */
  ...
  /**
   * Verify that removing a nonexistent edge does not detach any
   * connected vertices.
   */
  ...
  /**
   * Verify that removing an edge updates the associated vertices. Make
   * sure we can add the edge back.
   */
  ...
  /**
   * Verify that removing a vertex removes the associated edges. Make
   * sure we can add the vertex back.
   */
  public void testRemoveVertex() {
    ...
    ArrayList<Vertex<String>> neighbors =
        ( ArrayList<Vertex<String>> ) graph.getNeighbors( v1 );
  ...
  /**
   * Verify that DFSSearcher and BFSSearcer can find a path that exists
   * in the graph and does not find a path that does NOT exist.
   */
  public void testPath() {
    ...
    graph.addVertex( v2 );
    DFSSearcher<String> dfs = new DFSSearcher<String> ( graph );
    ...
    BFSSearcher<String> bfs = new BFSSearcher<String> ( graph );
  ...
  /**
   * Verify that DFSSearcher gets the vertices on the path from the source
   * to the destination
   */
  public void testGetPath() {
    v1 = new Vertex<String> ( "A" );
    graph.addVertex( v1 );
    Vertex<String> origin = v1;
    v2 = new Vertex<String> ( "B" );
    graph.addVertex( v2 );
    graph.addEdge( v1, v2 ); // A -> B
    v3 = new Vertex<String> ( "C" );
    ...
    graph.addVertex( v4 );
    graph.addEdge( v4, v2 ); // D -> B
    Vertex<String> dest = new Vertex<String> ( "E" );
    ...
    ArrayList<Vertex<String>> thepath =
