...
public class DFSSearcher<T> {
  ...
  private void doDFS( Vertex<T> origin, Vertex<T> target ) {
    pathFound = false;
  ...
  private void dfs( Vertex<T> origin, Vertex<T> target ) {
    if ( origin == target ) {
      pathFound = true;
      return;
    }
    Stack<Vertex<T>> neighborsToVisit = new Stack<Vertex<T>> ();
    ...
    while ( !neighborsToVisit.empty() ) {
      Vertex<T> v = neighborsToVisit.pop();
